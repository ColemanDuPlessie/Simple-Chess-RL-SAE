...
class QNetAutoencoder(nn.Module):
    def __init__(
        ...
        if topk_activation and loss_sparsity_term > 0:
            warnings.warn("WARNING: You have initialized an autoencoder with an l1 sparsity term in the loss and a topk activation function. Both of these are supposed to encourage sparsity, but you only really need one of them at a time.")
    ...
    def topk_act_func(self, in_tensor):
        values, indices = t.topk(in_tensor, k=self.k, dim=-1)
        mask = t.zeros_like(in_tensor)
